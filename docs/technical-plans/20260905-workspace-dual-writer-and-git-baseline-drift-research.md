...
3. **reference-check**：对照 letta（整仓 git commit 原子）/ orca（expectedRuntimeFence）/ herdr（结构态+版本化+原子写）/ opencode（content-addressed 快照）的「单一权威 + 单调代际」，判定「双写者」与「双基线」是否违反共同原则、参考项目怎么解决。
