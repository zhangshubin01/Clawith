...
| S5 | legacy `execute_tool` 门控只传 `actor_user_id`、缺 `actor_agent_id` → a2a 规则在 legacy/ACP 链式路径不生效 | ✅ **已出修复方案**（`20260907-legacy-seam-actor-agent-id-fix-plan.md`）→ 定性 **P2 预防**：Option C 零代码（文档化 seam 不对称 + 精准触发条件），否决 Option A（化妆式修补），Option B（6 函数完整透传）为升级路径。**台账纠正**：a2a 文件写是真实高频流量（20 run / 433 次，write 198/edit 157/move 60/delete 18）但全走 durable seam 且 `tool_permission_denied`=0 从未误拒，legacy 账本零 a2a 文件写 → latent 非 live |
