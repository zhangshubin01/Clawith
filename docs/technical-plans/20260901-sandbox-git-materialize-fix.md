...
**上线后缺陷（2026-09-03 实锤，已修复）——脱敏把绑定凭证从仓库里洗掉了**：`redact_git_secrets` 的 docstring 承诺「sandbox 保留真实 token 以便 pull/push 工作、storage 永不落 token」，但只实现了后半边——发布路径剥 token，**物化路径的重新注入从未实现**。绑定 init（`gitlab_workspace._apply_repo_config`）把 PAT 写成 `.git/config` 的 `url."http://oauth2:PAT@host/".insteadOf`，该凭证只活在 storage 那份 config 里；部署后第一个 flush（trigger run e21b6ac7，14:20:39Z，mtime 实锤）就把它剥成 `url."http://host/".insteadOf` 自引用空规则。此后每次物化都从这份已洗白的 config 复制 → 沙箱无任何凭证通道（无 env/helper/全局 config），agent 持续报「找不到凭证」；用户重存绑定救活一个 run，下一次 flush 又洗掉（脱敏不对称的死循环）。**修复（补全设计缺的半边）**：`gitlab_workspace.inject_credentials_into_temp_workspace`——`_prepare_temp_workspace` 在 `_drop_incomplete_git_dirs` 之后调用：副本存在完整 `.git`（有 HEAD）且绑定存在时，从 ChannelConfig 解密 PAT 复用 `_apply_repo_config` 写回**副本**的 `.git/config`。安全闭环不变：flush 在 hash 前先 redact，`redact(注入后 config) == storage base` → CAS 判无变化跳过，storage 永远干净；快路径（无 .git/无绑定/解密失败/仓库名不符）静默 no-op、零 DB 零 git 子进程，注入函数绝不抛出。测试 +8（`tests/test_gitlab_workspace.py`，FakeGit/FakeQueryDao monkeypatch 风格同现有）。
