...
1. **检测信号从签名迁移到「证据增益 + 材料变迁」**：判定「任务/世界状态是否前进」，非「动作是否相同」。增益分按工具键表（read 新 (path,content_hash)+1、execute_code 结果哈希变化 +1 / 失败→成功 +2、write 真实变更 +3、external_write +3），加 look-only 上限（连续 6 轮纯观察强制归零，封 git 绕圈 / 读遍新文件游走）。确定性、零 LLM 成本（不采用 gemini-cli 的语义复核，留作成本敏感时的可选项）。
...
- 正：根治「换姿势做同一件事」——git 巡检类命令永不产生材料变迁，天然命中；逐文件游走被 look-only 上限封死；信号落在客观副作用证据上，比 Reasonix 的「命令首次=+1」更根本（Reasonix 自己承认该盲区）。确定性、零增量模型调用。
