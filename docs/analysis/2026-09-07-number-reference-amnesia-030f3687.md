...
1. **呈现侧·编号命名空间漂移（结构性）**：上一轮 run（`fc4f38ce`「还有什么可优化的？」）在 closing answer 里用**新编号 1–8 重新聚合**了 99 项清单，产出「8 项 P2 可优化候选」（1=GitLab CI、2=lint……）。但 R1 落盘时按「延续编号」把这 8 项存为 **92–99**（`memory/清单.md` line 100–107，实测 `92. GitLab CI 缺失`、`93. 从未实际跑过 lint`、…、`99. allowBackup`）——呈现编号 1–8 与存储编号 92–99 **必然分叉**（`merge_list_items` 故意忽略 incoming 编号、从 max 继续，这是 R1 append-only 设计与「呈现子集视图」的固有张力）。
2. **上下文侧·注入窗口截断（结构性）**：下一 run 的上下文只由「持久化产物」重建——R3 注入（`cross_session_retrieval.py:85`）只注入 `section.items[:MAX_INJECTED_ITEMS]`（`MAX_INJECTED_ITEMS=20`）——**前 20 项**，92–99 号项在窗口外（Langfuse 埋点模型输入里 0 次命中「GitLab CI / 从未实际跑过 lint / allowBackup」；注入末尾虽标「仅列出前 20 项；完整内容见 memory/清单.md」，但模型没去读文件，直接锚定注入窗口里的 #1/#2=沙箱 git / 宿主 .git）。
...
一句话：**呈现 1–8、存储 92–99、注入 1–20 三套编号并存，旧契约又把「注入窗口」当权威且禁止猜测**——三层合力把「先做 #1+#2」导向了错误的清单（沙箱 git / 宿主 .git，而非 GitLab CI / lint）。

> **修正记录（2026-09-07 补充取证）**：初版曾称 8 项 P2「未落盘/被丢弃」，实测 `memory/清单.md` line 100–107 后修正——8 项**已落盘为 92–99**（R1 正常），真正断裂是「呈现重编 1–8」与「注入窗口只取前 20 项」两层。详见 `docs/technical-plans/20260907-number-reference-amnesia-fix-plan.md` §2.1。
...
- 09-07 案例：8 项 P2 **已落盘为 92–99**（R1 正常），但呈现时被**重新编号为 1–8**，且 R3 注入只取**前 20 项**、92–99 在窗口外——即便落地 R5 v2 的「解析优先级 ①本对话最近呈现的清单」，也会因「8 项视图不在注入窗口」而无法命中，只能靠「解析优先级 ②（文件权威，read_file 完整清单）」或「兜底消歧」追问兜底。比 09-04 案例多暴露「注入窗口截断」这一层。

因此修复需覆盖两维（已在 `docs/technical-plans/20260907-number-reference-amnesia-fix-plan.md` 展开）：
1. **编号同源 / 解析权威收口**：R5 v2 三条款（编号同源 + 解析优先级 + 兜底消歧）落地，且解析优先级补「注入仅为前 20 项索引、非权威全文，超出窗口先 read_file 再判定」——治「呈现 1–8 / 存储 92–99 / 注入 1–20 三套编号分叉 + 静默锚定注入窗口」。
2. **注入窗口尾部/最近呈现项**（后续观察项，本次最小改动不做）：`cross_session_retrieval.py:270` 的 `items[:MAX_INJECTED_ITEMS]` 改为尾部或最近呈现项，待 eval 量化 read_file/追问率后再定。
