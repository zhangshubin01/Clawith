...
外部渠道投递 Outbox：channel_deliveries
```

六张表都需要持久化保存，但不承担 LangGraph checkpoint 的职责：
...
- `channel_deliveries` 保存已生成 ChatMessage 的外部渠道投递任务；失败只重试 Provider 发送，不恢复或重跑 Graph。
...
| `channel_deliveries` | 保存已持久化 ChatMessage 到外部 Provider 的可靠投递任务。 | 与 ChatMessage 同事务创建，独立 claim / retry；不保存执行状态。 |
...
channel_deliveries 负责外部 Provider 的独立投递重试
```
...
| `correlation_id` | 跨 Run 等待与回调使用的稳定关联 ID，可为空。 |
...
| `attempt_count` | Command 被领取的次数；达到上限前必须先与 checkpoint 对账。 |
...
`delivery_succeeded / delivery_failed` 表示 Clawith 产品交付事务是否成功创建用户可见 ChatMessage；`channel_delivery_delivered / channel_delivery_failed` 表示该消息是否被外部 Provider 最终确认。外部 Provider 仍在重试时，Run 可以已经 terminal，但 `delivery_status` 保持 `pending`。
...
| `lease_owner` / `lease_expires_at` | 工具执行预占者及租约过期时间。 |
| `started_at` | 开始时间。 |
| `completed_at` | 完成时间。 |
| `updated_at` | 最近状态更新时间。 |
...
### 4.6 channel_deliveries

`channel_deliveries` 是外部渠道的窄 Transactional Outbox。它只负责把已经生成并持久化的 ChatMessage 送到飞书、钉钉、企业微信、微信、WhatsApp、Slack、Discord 或 Microsoft Teams，不是第二套执行状态机。
...
|-|-|
| `id` | 稳定 Delivery ID。 |
| `tenant_id` / `run_id` / `agent_id` / `session_id` / `message_id` | 产品归属与已持久化消息引用。 |
| `channel` | 外部 Provider 类型。 |
| `target` | 版本化的小型路由信息；动态 webhook / interaction token 视为敏感数据，不写日志或事件。 |
| `idempotency_key` | 与 ACK、waiting 或 terminal delivery request 一致的幂等键。 |
| `status` | `pending` / `claimed` / `delivered` / `failed`。 |
| `attempt_count` / `next_attempt_at` | 自动重试计数和下一次时间。 |
| `claimed_by` / `claim_expires_at` | Delivery Worker claim。 |
| `provider_message_id` | Provider 确认返回的消息 ID，可为空。 |
| `last_error_code` / `last_error` | 脱敏错误。 |
| `created_at` / `updated_at` / `delivered_at` | 时间戳。 |

ChatMessage、`delivery_succeeded` 产品事件和 `channel_deliveries` 必须在同一 PostgreSQL 事务提交。Delivery Worker 只读取该消息并调用 Provider；成功后更新 outbox 与 `agent_runs.delivery_status`，失败按退避重试，达到上限后写 `channel_delivery_failed`。任何 claim、重试或失败分支都不得调用 `start_run`、`resume_run`、Graph node 或 Checkpointer。

Provider API 通常不能提供跨网络事务的 exactly-once。若 Provider 已接收消息但进程在落库前崩溃，重试可能产生重复消息；支持稳定 Provider 幂等 ID 的渠道必须复用 `channel_deliveries.id`，其余渠道按 at-least-once 交付并保留可审计结果。多段消息在中途失败时也存在已发送分段重复风险，v1 不为此引入第二套分段状态机。
...
6. 迁移期保留新 Run 入口开关；未开放入口显式失败，不执行旧 tool loop。一个 run 创建后也不得切换 runtime。
...
6. WebSocket 消费 Runtime Event；非流式渠道入口提交版本化投递路由，最终结果由 `channel_deliveries` Worker 发送。
...
系统不立即物理删除该 session 关联的 `session_context_states`、`agent_runs`、`agent_run_commands`、`agent_run_events`、`agent_tool_executions`、`channel_deliveries` 和 checkpoint，但必须将其从 Runtime Context 查询和注入路径中排除。
...
下述 Phase 0–5 描述的是 Runtime 能力和各执行入口的渐进迁移，不代表统一聊天 Schema 继续处于双写状态。Runtime 入口可以使用 feature flag 分阶段开放新的 LangGraph Run；未开放的入口必须显式失败，不能回退旧 tool loop。同一个 run 创建后不得跨 runtime 切换，历史 `runtime_type = legacy` 记录也不由新 Worker 推进。

### Phase 0：Schema、迁移与依赖基线

- 编写并验证统一聊天模型迁移、六张 Runtime 表、群 mention 调度字段、Model Capability 字段及全部约束和索引。
...
- 迁移飞书及其他渠道。
- 渠道入口只保留消息解析、session 解析和 Runtime intake；外部结果由统一 `channel_deliveries` Worker 可靠投递，不再维护入口级等待任务或 tool loop。
...
- 同一 Agent 已经创建的多条群 mention 业务 Run 按 Message Position 通过 scheduling lane 串行；该 lane 不限制 Direct、Task、Trigger、Heartbeat 等其他 Run。
- 多 Agent 消息在 Planning 完成并创建具体业务 Run 前不占目标 Agent lane；v1 接受 Planning 期间后到的单 Agent Run 先进入 lane，只承诺已经进入 lane 的业务 Run 不并发。
...
<callout emoji="✅">
**最终结论：**LangGraph 负责完整执行生命周期，是唯一执行状态源；Clawith 保留产品事实、可靠 Command、Session Context、工具副作用 receipt、外部投递 Outbox、交付事实和可重建查询投影。两层不双写同一种权威状态。
</callout>
...
└─ Delivery Worker → 已持久化 ChatMessage → 外部 Provider
                      ↓
...
| 层 | 主要职责 | 不负责 |
|-|-|-|
| Clawith 产品层 | 租户和 Agent 权限、任务归属、来源入口、A2A 委托、可靠 Command、外部投递 Outbox、摘要查询、证据引用、交付状态和产品投影 | 不自行实现 Run 生命周期、waiting、node checkpoint、pending writes 和精确恢复 |
...
#### channel_deliveries：外部渠道可靠投递

`channel_deliveries` 与 assistant/system ChatMessage 在同一产品事务创建。Delivery Worker 只读取已持久化消息并调用 Provider，自动领取、退避重试并记录最终确认；它不读取 checkpoint 路由，不恢复 Run，也不重新执行工具。Provider 无法提供幂等发送时按 at-least-once 处理并明确保留重复消息风险。
...
每个阶段必须独立灰度和验收；在所有生产入口开放前，未开放入口保持 fail closed。新后端不保留 legacy 执行 path，同一个 run 也不得跨 runtime 切换。
...
<callout emoji="💡">
**范围说明：**旧发布版本不具备 checkpoint 级精确恢复；完成维护窗口切换的新后端只使用 LangGraph Runtime，不与旧执行路径混跑。恢复能力以实际 node 边界、checkpoint durability、thread 推进互斥和工具幂等实现为准。
</callout>
...
13. **执行与交付分离：**run completed 后即使渠道交付失败，也只领取或重试同一 `channel_deliveries` 记录，不重新执行任务；进程重启后仍可继续投递。
14. **权限不扩大：**checkpoint、Session Context、A2A context 和工具结果继续遵守 tenant、agent、session 和 origin 权限边界。
15. **可灰度开放：**新的 LangGraph Run 可按 Agent 或入口开放；未开放入口显式失败，已有 LangGraph Run 不受开关影响且不允许中途回退 legacy。
...
18. **群 mention 有序且不全局阻塞：**同一 Agent 已经创建的多条群 mention 业务 Run 按 Message Position 串行且不并发，Direct、Task、Trigger、Heartbeat 等其他 Run 不进入该调度 lane；Planning 创建业务 Run 之前不占 lane。
