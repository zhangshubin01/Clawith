...
回归门禁包括：非空/空白纯文本 finish、跨 Run draft/repair 过滤与当前 Run repair 保留、onboarding 并发/重挂载/失败重试/错 session、异步 MCP pending/replay/poll settle/finish block、旧 pending receipt 升级恢复、lifecycle stream 收口、Channel checkpoint resume、Web completion message refresh 和 Docker rollout policy 透传，以及 Chat、Group、Tool、Context 相邻路径。2026-07-16 当前全量结果为 backend `1907 passed`、frontend `26 passed`，frontend `tsc && vite build` 通过；真实 PostgreSQL migration、3010 部署和线上端到端证据在部署步骤单独记录，不能由单元测试替代。
