...
2. `/ws/group/{group_id}` 的路径、认证方式、close code 约定是否照上述执行？权限失效（群解散、成员被移出）时用哪个 close code——前端需要据此区分「可重连」与「不再重连」。
3. `after` 参数是否按上述语义实现？「是否还有更多」用方案 A（条数 == limit）还是 B（`has_more`）？
4. `modified_at` 是否统一成 ISO 8601？

2、3 不阻塞前端开工（前端先用轮询兜底），但直接影响群聊体感。1 是产品残缺。
