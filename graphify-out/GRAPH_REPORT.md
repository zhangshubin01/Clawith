...
## Corpus Check
- 244 files · ~381,039 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 3264 nodes · 13550 edges · 110 communities detected
- Extraction: 32% EXTRACTED · 68% INFERRED · 0% AMBIGUOUS · INFERRED: 9221 edges (avg confidence: 0.5)
...
- `Tenant-scoped key-value settings.` --uses--> `Base`  [INFERRED]
  backend/app/models/tenant_setting.py → backend/app/database.py

## Communities

### Community 0 - "Community 0"
Cohesion: 0.03
Nodes (326): DailyTokenUsage, CompanyCreateRequest, PlatformSettingsUpdate, Platform Admin company management API.  Provides endpoints for platform admins t, Create a new company and generate an admin invitation code (max_uses=1)., Enable or disable a company., Get daily platform metrics within a date range.      Returns per-day: companies,, Get Top 20 token consuming companies and agents. (+318 more)

### Community 1 - "Community 1"
Cohesion: 0.05
Nodes (352): get_agent_activity(), get_conversation_messages(), list_conversations(), Activity log API — view agent work history., Get messages for a specific conversation., Get recent activity logs for an agent., List all conversation partners for this agent (web users + other agents)., Agent (+344 more)
...
Cohesion: 0.04
Nodes (192): CompanyCreateResponse, CompanyStats, create_company(), get_enhanced_metrics(), get_platform_leaderboards(), get_platform_settings(), get_platform_timeseries(), list_companies() (+184 more)

### Community 4 - "Community 4"
Cohesion: 0.02
Nodes (150): AgentActivityLog, Activity log model for tracking agent actions., Records every action taken by a digital employee., Rolled up token consumption per agent per day for time-series analytics., log_activity(), Activity logger — simple async function to record agent actions., Record an agent activity. Fire-and-forget, never raises., Audit log, approval request, chat message, and enterprise info models. (+142 more)

### Community 5 - "Community 5"
Cohesion: 0.03
Nodes (103): AioSandboxBackend, aio-sandbox backend.      Connects to aio-sandbox (https://github.com/agent-infr, Check if aio-sandbox service is available., Execute code using aio-sandbox., AuditAction, Helper to write audit log entries from background services., Write audit log for role-related events.      Args:         action: Role action, Write audit log for tenant-related events.      Args:         action: Tenant act (+95 more)
...
Cohesion: 0.04
Nodes (98): create_template(), delegate_task(), DelegateRequest, delete_template(), get_agent_metrics(), get_template(), handover_agent(), HandoverRequest (+90 more)
...
Cohesion: 0.13
Nodes (46): generate_agent_api_key(), generate_api_key(), _get_agent_by_key(), get_setup_guide(), _hash_key(), heartbeat(), poll_messages(), Gateway API for OpenClaw agent communication.  OpenClaw agents authenticate via (+38 more)
...
Cohesion: 0.12
Nodes (24): ClawithPlugin, ClawithPlugin, ClawithAcpPlugin, ClawithMcpPlugin, ClawithSuperpowersPlugin, load_plugins(), 扫描 plugins/ 目录，加载每个含 plugin.json 的插件。, Register the plugin with the FastAPI app. (+16 more)
...
## Knowledge Gaps
- **275 isolated node(s):** `Clawith ACP Thin Client — IDE 侧瘦客户端（JetBrains Agent Client Protocol）  通过 WebSock`, `Database connection and session management.`, `SQLAlchemy declarative base.`, `Dependency for getting async database sessions.`, `Centralized logging configuration using loguru.` (+270 more)
  These have ≤1 connection - possible missing edges or undocumented components.
- **Thin community `Community 90`** (2 nodes): `update_schema.py`, `main()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 91`** (2 nodes): `remove_old_tool.py`, `run()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 92`** (1 nodes): `check_circular_imports.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 93`** (1 nodes): `merge_graphify.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 94`** (1 nodes): `vite.config.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 95`** (1 nodes): `vite-env.d.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 96`** (1 nodes): `index.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 97`** (1 nodes): `向 FastAPI app 注册路由、启动钩子等。`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 98`** (1 nodes): `Send a completion request and return the full response.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 99`** (1 nodes): `Send a streaming request and return the aggregated response.          Implementa`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 100`** (1 nodes): `scripts____init__.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 101`** (1 nodes): `从 dict 构建 SandboxConfig，支持字段级 fallback。          Args:             config: 工具配置`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 102`** (1 nodes): `Backend name for identification.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 103`** (1 nodes): `Backend name for identification.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 104`** (1 nodes): `Execute code in the sandbox.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 105`** (1 nodes): `Check if the sandbox backend is healthy.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 106`** (1 nodes): `Get the capabilities of this sandbox backend.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 107`** (1 nodes): `O3: Log IDE-provided MCP servers so we know what's available (not yet forwarded`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 108`** (1 nodes): `setup-win.ps1`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 109`** (1 nodes): `run-win.ps1`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `User` connect `Community 0` to `Community 1`, `Community 3`, `Community 4`, `Community 5`, `Community 8`, `Community 9`, `Community 12`, `Community 13`, `Community 15`, `Community 21`, `Community 24`, `Community 25`, `Community 29`, `Community 34`, `Community 37`, `Community 43`, `Community 55`, `Community 57`, `Community 89`?**
  _High betweenness centrality (0.260) - this node is a cross-community bridge._
- **Why does `Agent` connect `Community 1` to `Community 0`, `Community 34`, `Community 3`, `Community 4`, `Community 5`, `Community 37`, `Community 8`, `Community 9`, `Community 43`, `Community 44`, `Community 15`, `Community 53`, `Community 24`, `Community 25`, `Community 29`?**
  _High betweenness centrality (0.138) - this node is a cross-community bridge._
- **Why does `SandboxConfig` connect `Community 1` to `Community 3`, `Community 5`?**
...
- **Are the 547 inferred relationships involving `Agent` (e.g. with `Seed data script — creates initial admin user and built-in templates.` and `Create tables and seed initial data.`) actually correct?**
  _`Agent` has 547 INFERRED edges - model-reasoned connections that need verification._
