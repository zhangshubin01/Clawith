...
## Corpus Check
- 310 files · ~518,980 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 3871 nodes · 14883 edges · 153 communities detected
- Extraction: 34% EXTRACTED · 66% INFERRED · 0% AMBIGUOUS · INFERRED: 9756 edges (avg confidence: 0.5)
- Token cost: 0 input · 0 output

## God Nodes (most connected - your core abstractions)
1. `User` - 836 edges
2. `Agent` - 598 edges
3. `IdentityProvider` - 388 edges
4. `ChannelConfig` - 339 edges
5. `Tenant` - 324 edges
6. `OrgMember` - 322 edges
7. `LLMModel` - 314 edges
8. `SystemSetting` - 311 edges
9. `Identity` - 278 edges
10. `ChatMessage` - 278 edges
...
Cohesion: 0.02
Nodes (312): ApprovalRequest, EnterpriseInfo, ExternalUserInfo, Generic OAuth/SSO authentication provider framework.  This module provides a bas, Get or create IdentityProvider record., Find user by legacy provider-specific fields (if any)., Update existing user with new info from provider., Create new user from external identity. (+304 more)

### Community 1 - "Community 1"
Cohesion: 0.05
Nodes (382): get_agent_activity(), get_conversation_messages(), list_conversations(), DailyTokenUsage, Activity log API — view agent work history., Get messages for a specific conversation., Get recent activity logs for an agent., List all conversation partners for this agent (web users + other agents). (+374 more)

### Community 2 - "Community 2"
Cohesion: 0.01
Nodes (76): fetchJson(), handleCreate(), handleSendTestEmail(), handleToggle(), handleToggleSetting(), loadCompanies(), saveEmailConfig(), saveEmailTemplates() (+68 more)

### Community 3 - "Community 3"
Cohesion: 0.04
Nodes (206): authorize(), bind_identity(), change_password(), check_duplicate(), forgot_password(), get_email_hint(), get_me(), get_my_tenants() (+198 more)

### Community 4 - "Community 4"
Cohesion: 0.01
Nodes (182): AgentActivityLog, Activity log model for tracking agent actions., Records every action taken by a digital employee., Rolled up token consumption per agent per day for time-series analytics., log_activity(), Activity logger — simple async function to record agent actions., Record an agent activity. Fire-and-forget, never raises., Audit log, approval request, chat message, and enterprise info models. (+174 more)

### Community 5 - "Community 5"
Cohesion: 0.02
Nodes (125): AboutCommand, about command - display version information about Clawith ACP., AcpCommand, AgentsCommand, AioSandboxBackend, aio-sandbox backend.      Connects to aio-sandbox (https://github.com/agent-infr, Check if aio-sandbox service is available., Execute code using aio-sandbox. (+117 more)

### Community 6 - "Community 6"
Cohesion: 0.02
Nodes (119): call_agent_llm(), call_agent_llm_with_tools(), call_llm(), call_llm_with_failover(), _check_tool_requires_args(), _convert_messages_for_vision(), FailoverGuard, _get_agent_config() (+111 more)
...
Cohesion: 0.03
Nodes (66): AgentSideConnection, ndJson stream connection over WebSocket for ACP.  This module handles the ndJson, ACP Agent-side connection wrapping ndJson stream over WebSocket., Read one line from stdin, parse as JSON., Send one JSON message as a line., Close the connection., RPC: ask IDE to read a text file., RPC: ask IDE to write a text file. (+58 more)

### Community 10 - "Community 10"
Cohesion: 0.04
Nodes (97): create_template(), delegate_task(), DelegateRequest, delete_template(), get_agent_metrics(), get_template(), handover_agent(), HandoverRequest (+89 more)
...
Cohesion: 0.05
Nodes (60): AuditAction, Helper to write audit log entries from background services., Write audit log for role-related events.      Args:         action: Role action, Write audit log for tenant-related events.      Args:         action: Tenant act, Standard audit action types., Internal method to write audit log., Write a single audit log entry using raw SQL.      Uses raw SQL to avoid ORM for, Write audit log for identity-related events.      Args:         action: Identity (+52 more)
...
Cohesion: 0.08
Nodes (17): ABC, BaseAuthProvider, DingTalkAuthProvider, FeishuAuthProvider, MicrosoftTeamsAuthProvider, WeComAuthProvider, AuthProviderRegistry, Authentication provider registry and factory.  This module provides a centralize (+9 more)
...
Cohesion: 0.14
Nodes (42): _get_agent_by_key(), get_setup_guide(), _hash_key(), heartbeat(), poll_messages(), Gateway API for OpenClaw agent communication.  OpenClaw agents authenticate via, OpenClaw agent reports the result of a processed message., Pure heartbeat ping — keeps the OpenClaw agent marked as online. (+34 more)
...
Cohesion: 0.13
Nodes (18): _acp_await_client_permission(), _acp_verbose(), acp_websocket(), _acp_ws_envelope(), _build_acp_user_turn_from_ws(), _custom_execute_tool(), _generate_structured_diff_blocks(), _hydrate_if_needed_acp() (+10 more)
...
- **Thin community `Community 116`** (2 nodes): `test_async.py`, `test_manager()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 117`** (2 nodes): `update_schema.py`, `main()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 118`** (2 nodes): `remove_old_tool.py`, `run()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 119`** (2 nodes): `ws-protocol.test.js`, `runTests()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 120`** (1 nodes): `check_circular_imports.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 121`** (1 nodes): `verify_superpowers_tests.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 122`** (1 nodes): `merge_graphify.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 123`** (1 nodes): `vite.config.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 124`** (1 nodes): `vite-env.d.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 125`** (1 nodes): `setup-win.ps1`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 126`** (1 nodes): `run-win.ps1`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 127`** (1 nodes): `test_import.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 128`** (1 nodes): `test_adapter_direct.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 129`** (1 nodes): `向 FastAPI app 注册路由、启动钩子等。`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 130`** (1 nodes): `Command name (e.g., "about").`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 131`** (1 nodes): `Brief description for help output.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 132`** (1 nodes): `Alternative names for this command.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 133`** (1 nodes): `Nested subcommands (for "extensions list" style).`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 134`** (1 nodes): `Execute the command with given arguments.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 135`** (1 nodes): `scripts____init__.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 136`** (1 nodes): `Send a completion request and return the full response.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 137`** (1 nodes): `Send a streaming request and return the aggregated response.          Implementa`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 138`** (1 nodes): `从 dict 构建 SandboxConfig，支持字段级 fallback。          Args:             config: 工具配置`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 139`** (1 nodes): `Backend name for identification.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 140`** (1 nodes): `Backend name for identification.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 141`** (1 nodes): `Execute code in the sandbox.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 142`** (1 nodes): `Check if the sandbox backend is healthy.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 143`** (1 nodes): `Get the capabilities of this sandbox backend.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 144`** (1 nodes): `Enqueue a new permission request for later approval.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 145`** (1 nodes): `Get all pending permission requests for a session.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 146`** (1 nodes): `Get a specific pending permission request by ID.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 147`** (1 nodes): `Process a permission decision (grant/deny).`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 148`** (1 nodes): `Wait for a decision on a permission request.          Returns True if granted, F`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 149`** (1 nodes): `Clear all pending requests for a session.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 150`** (1 nodes): `Count pending requests for a session.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 151`** (1 nodes): `Read content from a text file.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 152`** (1 nodes): `Write content to a text file.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `User` connect `Community 0` to `Community 1`, `Community 3`, `Community 4`, `Community 5`, `Community 6`, `Community 8`, `Community 10`, `Community 11`, `Community 14`, `Community 15`, `Community 17`, `Community 23`, `Community 24`, `Community 28`, `Community 29`, `Community 32`, `Community 37`, `Community 40`, `Community 50`, `Community 65`, `Community 69`, `Community 75`?**
  _High betweenness centrality (0.240) - this node is a cross-community bridge._
- **Why does `Agent` connect `Community 1` to `Community 0`, `Community 3`, `Community 4`, `Community 5`, `Community 6`, `Community 8`, `Community 10`, `Community 11`, `Community 17`, `Community 23`, `Community 28`, `Community 29`, `Community 32`, `Community 38`, `Community 40`, `Community 50`, `Community 52`, `Community 61`, `Community 75`?**
  _High betweenness centrality (0.136) - this node is a cross-community bridge._
- **Why does `ChannelConfig` connect `Community 1` to `Community 0`, `Community 3`, `Community 4`, `Community 36`, `Community 38`, `Community 37`, `Community 40`, `Community 10`, `Community 12`, `Community 17`, `Community 30`?**
  _High betweenness centrality (0.053) - this node is a cross-community bridge._
- **Are the 833 inferred relationships involving `User` (e.g. with `Seed data script — creates initial admin user and built-in templates.` and `Create tables and seed initial data.`) actually correct?**
  _`User` has 833 INFERRED edges - model-reasoned connections that need verification._
- **Are the 595 inferred relationships involving `Agent` (e.g. with `Seed data script — creates initial admin user and built-in templates.` and `Create tables and seed initial data.`) actually correct?**
  _`Agent` has 595 INFERRED edges - model-reasoned connections that need verification._
- **Are the 385 inferred relationships involving `IdentityProvider` (e.g. with `Base` and `Create a new SSO scan session for QR code login.`) actually correct?**
  _`IdentityProvider` has 385 INFERRED edges - model-reasoned connections that need verification._
- **Are the 336 inferred relationships involving `ChannelConfig` (e.g. with `Seed data script — creates initial admin user and built-in templates.` and `Create tables and seed initial data.`) actually correct?**
  _`ChannelConfig` has 336 INFERRED edges - model-reasoned connections that need verification._