...
deleted the dead 9-key dict and kept the effective one as the sole definition.

Since then the Maintainer gate (``resolve_file_modify_permission``) took over the
four file-modifying document tools (write_file/edit_file/delete_file/move_file),
so the map now covers ONLY execute_code/execute_command.
...
2-key set will fail loudly if anyone reintroduces a second dict that overwrites the
effective one (the final value would then deviate from the expected keys).
"""
...
# tool_name -> autonomy action_type. File tools (write/edit/delete/move) are
# gated by ``resolve_file_modify_permission`` (Maintainer gate) instead of this
# map; send_*/web_search/execute_code_e2b are typed or A2A-settled and never
# reach it.
EXPECTED_MAP = {
...
def test_tool_autonomy_map_is_exactly_the_effective_code_keys() -> None:
