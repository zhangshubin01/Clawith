...
def upgrade() -> None:
    ...
    if not _column_exists('workspace_file_revisions', 'scope_type'):
        op.add_column('workspace_file_revisions',
            sa.Column('scope_type', sa.String(20), nullable=False, server_default='agent'))
    if not _column_exists('workspace_file_revisions', 'scope_id'):
        op.add_column('workspace_file_revisions',
    ...
    if not _constraint_exists('workspace_file_revisions', 'ck_workspace_file_revisions_scope_type'):
        op.create_check_constraint(
            'ck_workspace_file_revisions_scope_type',
            'workspace_file_revisions',
            "scope_type IN ('agent', 'group')")
    if not _constraint_exists('workspace_file_revisions', 'ck_workspace_file_revisions_scope_identity'):
        op.create_check_constraint(
    ...
    if not _column_exists('workspace_edit_locks', 'scope_type'):
        op.add_column('workspace_edit_locks',
            sa.Column('scope_type', sa.String(20), nullable=False, server_default='agent'))
    if not _column_exists('workspace_edit_locks', 'scope_id'):
        op.add_column('workspace_edit_locks',
            sa.Column('scope_id', sa.UUID(), nullable=False, server_default=sa.text("'00000000-0000-0000-0000-000000000000'::uuid")))
    # Backfill: set scope_id = agent_id for all existing rows
    op.execute("UPDATE workspace_edit_locks SET scope_id = agent_id WHERE scope_id = '00000000-0000-0000-0000-000000000000'")
    if not _constraint_exists('workspace_edit_locks', 'ck_workspace_edit_locks_scope_type'):
        op.create_check_constraint(
            'ck_workspace_edit_locks_scope_type',
            'workspace_edit_locks',
            "scope_type IN ('agent', 'group')")
    if not _constraint_exists('workspace_edit_locks', 'ck_workspace_edit_locks_scope_identity'):
        op.create_check_constraint(
