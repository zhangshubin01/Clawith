...
🚫 **NEVER say you cannot access the internet or search the web.** You HAVE these capabilities — use them.""")
    ...
    if query:
        try:
            from app.services.openviking_client import search_enterprise, is_available
            if await is_available():
                enterprise_snippets = await search_enterprise(query=query)
                if enterprise_snippets:
                    _enterprise_text = "\n\n".join(enterprise_snippets)
                    logger.debug(f"[OpenViking] Retrieved {len(enterprise_snippets)} enterprise shared snippet(s)")
        except Exception as _ov_err:
            logger.debug(f"[OpenViking] enterprise search skipped: {_ov_err}")

    if _memory_text:
        dynamic_parts.append(f"\n## Memory\n{_memory_text}")

    if _enterprise_text:
        dynamic_parts.append(f"\n## Enterprise Shared Information\n{_enterprise_text}")
...
🚫 **NEVER say you cannot access the internet or search the web.** You HAVE these capabilities — use them.""")
    ...
    # --- Enterprise Shared Knowledge: OpenViking semantic retrieval ---
    if query:
        try:
            from app.services.openviking_client import search_enterprise, is_available
            if await is_available():
                ...
                if enterprise_snippets:
                    _enterprise_text = "\n\n".join(enterprise_snippets)
                    dynamic_parts.append(f"\n## Enterprise Shared Information\n{_enterprise_text}")
                    logger.debug(f"[OpenViking] Retrieved {len(enterprise_snippets)} enterprise snippet(s)")
        except Exception as _ov_err:
            logger.debug(f"[OpenViking] enterprise search skipped: {_ov_err}")

    # --- Skills semantic hint: surface the most relevant skill for this query ---
    if query and skills_text:
        try:
            from app.services.openviking_client import search_skills, is_available
            if await is_available():
                skill_hints = await search_skills(query=query)
                if skill_hints:
                    dynamic_parts.append(f"\n## Skill Recommendation\nBased on the current request, the most relevant skill is:\n\n{skill_hints[0]}")
                    logger.debug(f"[OpenViking] Skill hint injected for query")
        except Exception as _ov_err:
            logger.debug(f"[OpenViking] skill search skipped: {_ov_err}")
