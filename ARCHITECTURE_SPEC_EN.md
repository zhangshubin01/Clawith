...
- The company report view is rendered as an expandable list by period. Only per-item missing-report badges and per-item regenerate actions are shown; regenerate is only exposed when that specific report has `needs_refresh=true` after late submissions or corrections. One report stays expanded at all times so the report area never collapses into an empty state.
...
| 2026-04-19 | Refined OKR report browsing so one company report remains expanded at all times and toned dashboard OKR visuals down to a grayscale palette. |
